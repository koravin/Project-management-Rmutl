...
const CE04Upload = () => {
  ...
  // ฟังก์ชันสำหรับ ส่งเอกสาร CE04
  const handleCE04Upload = async () => {
    const docType = 'CE04'
    try {
      // ประกอบร่างชื่อใหม่
      ...
      if (!studentData && !advisor && instructor.length === 0) {
        Swal.fire({
          icon: 'error',
  ...
  const handlePreview = async FileName => {
    try {
      const previewResponse = await fetch('/api/download', {
        ...
        body: JSON.stringify({ fileName: FileName, docType: 'CE04' }),
        responseType: 'blob'
      })
  ...
  }, [advisor, instructor, studentData])
  ...
  const handleInstructorChange = event => {
    setInstructorData(event.target.value)
    setAdvisorData('')
    setStudentData('')
  }
    ...
    <div>
        ...
        <Card sx={{ mb: 0.5, borderRadius: 2, width: '70%' }}>
          ...
          <CardContent>
            <Grid container spacing={2} sx={{ display: 'flex', justifyContent: 'center' }}>
              ...
              <Grid item xs={12} sm={3}>
                <FormControl fullWidth>
                  <InputLabel id='Instructor-label'>อาจารย์ประจำวิชา</InputLabel>
                  <Select
                  ...
                  >
                    <MenuItem value={999}>อ.สัญญา อุทธโยธา</MenuItem>
                  </Select>
                </FormControl>
              </Grid>
            </Grid>
      ...
      <Box style={{ display: 'flex', justifyContent: 'center' }}>
        <Card style={{ width: '60%', borderRadius: 15 }}>
          ...
          >
            <PostAddIcon style={{ marginRight: '0.2rem', height: '5vh' }} /> อัปโหลดเอกสาร CE 04
          </Typography>
            ...
            <Grid container direction='row' justifyContent='center'>
              ...
              <Grid justifyContent='center'>
                <WhiteBlackButton
                  ...
                  onClick={() => {
                    handleCE04Upload()
                    setRefreshFlag(prevFlag => !prevFlag) // เรียกใช้ useEffect ใน CE04Record
                  }}
                >
                  ส่ง
                </WhiteBlackButton>
