...
const CE05Upload = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_API}api/project-mgt/getallonedocumenttype?preproject_id=${projectID}&document_type=CE05`
        )
  ...
  // ฟังก์ชันสำหรับ ส่งเอกสาร CE05
  const handleCE05Upload = async () => {
    const docType = 'CE05'
    try {
      // ประกอบร่างชื่อใหม่
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // console.log('ข้อมูล CE05', response.data)
  ...
  const handlePreview = async FileName => {
    try {
      const previewResponse = await fetch('/api/download', {
        ...
        body: JSON.stringify({ fileName: FileName, docType: 'CE05' }),
        responseType: 'blob'
      })
    ...
    <div>
      ...
      <Box style={{ display: 'flex', justifyContent: 'center' }}>
        <Card style={{ width: '60%', borderRadius: 15 }}>
          ...
          >
            <PostAddIcon style={{ marginRight: '0.2rem', height: '5vh' }} /> อัปโหลดเอกสาร CE 05
          </Typography>
            ...
            <Grid container direction='row' justifyContent='center'>
              ...
              <Grid justifyContent='center'>
                <WhiteBlackButton
                  ...
                  onClick={() => {
                    handleCE05Upload()
                    setRefreshFlag(prevFlag => !prevFlag) // เรียกใช้ useEffect ใน CE05Record
                  }}
                >
                  ส่ง
                </WhiteBlackButton>
