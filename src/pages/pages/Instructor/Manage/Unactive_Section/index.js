...
const Unactive_Section = () => {
  const router = useRouter() // router สร้าง path

  // รับค่าข้อมูล Api
  const [sectionData, setSectionData] = useState([])

  // รับค่าข้อมูลจาก Api Unctive Section
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${process.env.NEXT_PUBLIC_API}api/project-mgt/secs_unactive`)
...
export default Unactive_Section
