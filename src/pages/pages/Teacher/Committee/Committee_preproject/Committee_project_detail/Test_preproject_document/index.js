...
export default function Test_preproject_document({ project_id }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_API}api/project-mgt/document_one_preproject?preproject_id=${projectID}`
        )

        console.log('ข้อมูลเอกสาร Active', response.data.listDocument)

        const rowsWithDocumentInfo = response.data.listDocument.map((row, index) => ({
          ...row,
          document_id: index + 1
        }))

        setRowData(rowsWithDocumentInfo)
      } catch (error) {
        console.error(error)
      }
    }
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: params => (
        <Button
          variant='h6'
          style={{ color: 'pink' }}
          onClick={() => handlePreview(params.row.document_names, params.row.document_type)}
        >
          ...
        </Button>
      )
    },
    {
      ...
      width: 180,
      renderCell: params => (
        <Button variant='outlined' onClick={() => handleDownload(params.row.document_names, params.row.document_type)}>
          ดาวน์โหลด
        </Button>
      )
    }
  ...
  const handlePreview = async (FileName, DocumentType) => {
    try {
