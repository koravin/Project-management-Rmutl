...
export default function Edit_post({ open, handleClose, rowData }) {
  ...
  // เซตค่าข้อมูลเมื่อ Modal เปิดหรือปิด
  useEffect(() => {
    if (rowData && 'description' in rowData && 'header_name' in rowData) {
      setTopic(rowData.header_name)
      setPostData(rowData.description)
      setSubmitted(false)
    } else {
      console.log('Jongrai Error')
    }
  }, [rowData])
  ...
  return (
    <React.Fragment>
      <BootstrapDialog fullWidth onClose={handleClose} aria-labelledby='customized-dialog-title' open={open}>
        ...
        <DialogActions>
          <Button onClick={handleEditTopicClick}>แก้ไข</Button>
        </DialogActions>
      </BootstrapDialog>
    </React.Fragment>
  )
