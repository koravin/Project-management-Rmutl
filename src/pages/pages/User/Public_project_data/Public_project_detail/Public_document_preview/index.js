...
export default function Public_document_preview({ project_id }) {
  ...
  const [getCHType, setGetCHData] = useState('public') // รับค่าข้อมูลประเภทเอกสาร
  ...
  const handleCHUpload = async () => {
    ...
    try {
      ...
      const uploadResponse = await fetch('/api/public_document_upload', {
        method: 'POST',
        body
      })
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_API}api/project-mgt/getallonedocumenttypeproject?project_id=${projectID}&document_type=PUBLIC`
        )
  ...
  const handleDownload = async FileName => {
    ...
    try {
      const downloadResponse = await fetch('/api/public_document_download', {
        method: 'POST',
        headers: {
  ...
  const handlePreview = async FileName => {
    try {
      const previewResponse = await fetch('/api/public_document_download', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      >
        ...
        >
          <Card style={{ width: '100%', borderRadius: 15 }}>
            ...
            >
              <AccessTimeIcon style={{ marginRight: '0.2rem', height: '5vh' }} /> Document preview
            </Typography>
