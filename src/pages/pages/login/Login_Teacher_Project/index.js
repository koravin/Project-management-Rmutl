...
const Login_Teacher_Project = () => {
  // นำเข้าตัวsweetalert2
  const Swal = require('sweetalert2')

  // ตัวแปรเก็บข้อมูล user
  const [user, setUser] = useState('') // ตัวแปรเก็บข้อมูล user

  const [values, setValues] = useState({
    password: '',
    showPassword: false
  }) // ตัวแปรเก็บค่า password
  ...
  const handleInsertSubmit = e => {
    ...
    axios
      .post(`${process.env.NEXT_PUBLIC_API}api/project-mgt/login-teacher-project`, data)
      .then(response => {
        if (response.data.statusCode === 404) {
          ...
          // console.log('หา Id', response.data.data)

          // เก็บค่า JWT และ JWT Role ใน localStorage
          localStorage.setItem('jwtToken', response.data.jwt)
          localStorage.setItem('jwtRole', response.data.jwtRole)
          localStorage.setItem('jwtFirst_Name', response.data.data.first_name)
          localStorage.setItem('jwtLast_Name', response.data.data.last_name)
...
export default Login_Teacher_Project
